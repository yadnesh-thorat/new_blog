...
export default function AboutPage() {
  ...
  const displayTitle = (language === "en" && aboutContent?.titleEn)
    ? aboutContent.titleEn
    : (aboutContent?.title || "आमच्याबद्दल");
  const displayText = (language === "en" && aboutContent?.textEn)
    ? aboutContent.textEn
    : (aboutContent?.text || "");
      ...
      <main className="flex-grow transition-colors duration-300">
        ...
        <section className="mx-auto max-w-5xl px-4 sm:px-6 lg:px-8 pt-28 pb-8 animate-entrance">
          <div className="border-b border-border/40 pb-5 mb-8">
            <h3 className={`text-2xl font-bold ${language === "mr" ? "font-devanagari" : "font-geist-sans"}`}>
              {displayTitle}
            </h3>
            <p className={`text-sm text-muted-foreground mt-1 leading-relaxed ${language === "mr" ? "font-devanagari" : ""}`}>
              {displayText}
            </p>
          </div>
        </section>
